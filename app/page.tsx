import {ArrowRight,BriefcaseBusiness,CheckCircle2,Clock3,LockKeyhole,Mail,MapPin,Phone,ShieldCheck,Sparkles,UserRoundCheck,UsersRound,} from "lucide-react";

const phoneDisplay = "410-542-2255";const phoneHref = "tel:4105422255";const email = "hello@abacruit.com";const instagramHref = "https://www.instagram.com/abacruit/";

const roles = [["BCBAs", "Our specialty and most requested role."],["Clinical Directors", "Leadership candidates for growing ABA organizations."],["Regional Directors", "Multi-site clinical leadership and growth support."],["RBTs & BTs", "Direct service team members for client care."],["ABA Supervisors", "Experienced clinical support for your team."],["Operations Staff", "Schedulers, intake coordinators, and admin support."],];

const professionalBenefits = ["Confidential career search","Better-fit opportunities","Personalized career guidance","Salary and offer insights","Nationwide opportunities","No cost to candidates",];
...
function TrustBar() {const items = [["Specialized ABA Recruiting", ShieldCheck],["Nationwide Talent Network", UsersRound],["Personalized Service", UserRoundCheck],["Confidential & Professional", LockKeyhole],] as const;

return (<section className="border-y border-slate-200 bg-white"><div className="mx-auto grid max-w-7xl gap-5 px-5 py-6 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">{items.map(([label, Icon]) => (<div key={label} className="flex items-center gap-3"><Icon className="h-6 w-6 shrink-0 text-blue" /><span className="text-sm font-black text-slate-700">{label}</span></div>))}</div></section>);}

function FeaturedOpportunity() {return (<section id="opportunities" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-blue">Featured opportunity</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Current ABA Opportunities</h2><p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600">Explore selected opportunities or join the ABA Cruit Talent Network to hear about roles that better match your goals.</p></div><a href="#talent-network" className="inline-flex items-center font-black text-blue hover:text-navy">Not the right fit? Join the network <ArrowRight className="ml-2 h-5 w-5" /></a></div>
...
function ReferralCTA() {return (<section className="bg-soft py-20"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-8 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-soft lg:grid-cols-[1.1fr_.9fr]"><div className="p-8 md:p-12"><p className="text-sm font-black uppercase tracking-[0.22em] text-blue">ABA Cruit Referral Program</p>
...
function TalentNetwork() {return (<section id="talent-network" className="bg-soft py-20"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-10 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-soft lg:grid-cols-[.8fr_1.2fr] lg:p-12"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-blue">Not seeing the right opportunity?</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Join the ABA Cruit Talent Network</h2><p className="mt-5 text-lg leading-8 text-slate-600">You do not need to be actively job hunting. Tell us what matters to you once, and we’ll reach out when a relevant opportunity may be worth considering.</p>
...
function TwoPaths() {return (<section className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="grid gap-6 lg:grid-cols-2"><div className="rounded-[2rem] border border-slate-200 bg-warm p-8 shadow-sm"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-blue shadow-sm"><UserRoundCheck className="h-6 w-6" /></div><p className="text-sm font-black uppercase tracking-[0.18em] text-blue">ABA Professionals</p><h2 className="mt-2 text-3xl font-black">Find Your Next Opportunity</h2><div className="mt-5 space-y-3">{professionalBenefits.map((item) => (<div key={item} className="flex items-start gap-3 text-sm font-bold text-slate-700"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-blue" />{item}</div>))}</div><a href="/opportunities" className="mt-7 inline-flex items-center justify-center rounded-full border border-blue bg-white px-6 py-4 font-bold text-blue transition hover:bg-blue hover:text-white">Browse Opportunities <ArrowRight className="ml-2 h-5 w-5" /></a></div>
...
function Roles() {return (<section id="roles" className="mx-auto max-w-7xl px-5 pb-20 lg:px-8"><div className="mx-auto max-w-3xl text-center"><p className="text-sm font-black uppercase tracking-[0.22em] text-blue">Who we help fill</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Roles We Recruit For</h2></div>
...
function Footer() {return (<footer id="contact" className="bg-navy text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 md:grid-cols-[1fr_auto_auto] md:items-center lg:px-8"><div><div className="text-2xl font-black"><span className="text-blue">ABA</span> Cruit</div><div className="mt-1 text-xs font-semibold uppercase tracking-[0.16em] text-slate-300">Staffing & Recruiting</div></div>
...
function Input({ name, placeholder, type = "text" }: { name: string; placeholder: string; type?: string }) {return (<input
required
name={name}
type={type}
...
function Select({ name, children }: { name: string; children: React.ReactNode }) {return (<selectrequiredname={name}className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-4 text-sm text-navy outline-none transition focus focus focus/10"
